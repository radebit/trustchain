...
public class LoginController {
     ...
     * @return
     */
    @PostMapping("/geetestLogin")
    @ApiOperation(value = "极验用户登录", notes = "极验用户登录")
    public Result<JSONObject> geetestLogin(@RequestBody JSONObject jsonObject) {
        // 断言
        Assert.notNull(jsonObject.getString("clientType"), "客户端类型不能为空！");
        Assert.notNull(jsonObject.getString("secCode"), "验证串不能为空！");
        Assert.notNull(jsonObject.getString("challenge"), "验证唯一标识不能为空！");
        Assert.notNull(jsonObject.getString("username"), "用户名不能为空！");
        Assert.notNull(jsonObject.getString("password"), "密码不能为空！");
        Result<JSONObject> result = new Result<JSONObject>();
        // 校验验证码
